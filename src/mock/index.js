...
function updateCluster(id, updater) {
...
function updateClusterResources(id, updater) {
    return updateCluster(id, updater);
}
function recalcNamespaces(cluster) {
...
function summarizeOverview(cluster) {
    return {
...
Mock.mock('/api/resources/deployments', 'post', ({ body }) => {
    const { clusterId, namespace, name, image, replicas } = parseBody(body);
    const updated = updateClusterResources(clusterId, (cluster) => {
        ...
        const pods = Array.from({ length: replicas }).map((_, index) => ({
...
Mock.mock(/\/api\/resources\/deployments\/[^/]+\/scale$/, 'post', ({ url, body }) => {
    ...
    const { clusterId, replicas } = parseBody(body);
    const updated = updateClusterResources(clusterId, (cluster) => {
        const deployments = cluster.resources.deployments.map((item) => {
        ...
        const nextPods = Array.from({ length: replicas }).map((_, index) => ({
...
Mock.mock(/\/api\/resources\/deployments\/[^/]+\/delete$/, 'post', ({ url, body }) => {
    const segments = url?.split('/') ?? [];
    const deploymentId = segments[segments.length - 2] ?? '';
    const { clusterId } = parseBody(body);
    const updated = updateClusterResources(clusterId, (cluster) => {
...
Mock.mock('/api/resources/services', 'post', ({ body }) => {
    const { clusterId, namespace, name, type, ports, selector } = parseBody(body);
    const updated = updateClusterResources(clusterId, (cluster) => recalcNamespaces({
        ...cluster,
